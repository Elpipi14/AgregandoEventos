...
function agregarAlCarrito(productos) {
    ...
    const enCarrito = carrito.find(prod => prod.id == productos.id);

    if (!enCarrito) {
        carrito.push({...productos, cantidad: 1 });
    } else {

        const filterCarrito = carrito.filter(prod => prod.id != productos.id);
        carrito = [
            ...filterCarrito, {...enCarrito, cantidad: enCarrito.cantidad + 1 }
        ]
    }
    console.log(carrito)
}